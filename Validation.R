...
box_comp <- function(stage, label, y_axis, box_color) {
    ...
    validation_dataset %>% filter(Stage == stage) %>%
        ...
        geom_jitter(width = 0.3, alpha = 0.3, size = 2) +
...
# natural adults
b1 <- box_comp('TrapAdult', 'Natural Adults to Trap', 'Natural Adults', 'dodgerblue')
b2 <- box_comp('LGDSmolt', 'Natural LGD Smolt', 'LGD Smolt', 'dodgerblue')
b3 <- box_comp('Parr', 'Late Summer Parr', 'Parr', 'dodgerblue')
b4 <- box_comp('H1_TrapAdult', 'Hatchery Adults to Trap', 'Hatchery Adults', 'deeppink3')
b5 <- box_comp('H1_LGDSmolt', 'Hatchery LGD Smolt', 'Hatchery LGD Smolt', 'deeppink3')
b6 <- box_comp('H1_HatchRelease', 'Hatchery Smolt Released', 'Hatchery Release', 'deeppink3')
